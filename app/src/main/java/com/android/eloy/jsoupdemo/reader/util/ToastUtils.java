...
public class ToastUtils {
    ...
    public static Toast getSingleToast(int resId, int duration) { // 连续调用不会连续弹出，只是替换文本
        return getSingleToast(App.getContext().getResources().getText(resId).toString(), duration);
    }

    public static Toast getSingleToast(String text, int duration) {
        if (mToast == null) {
            mToast = new Toast(App.getContext());
            mToast.setView(LayoutInflater.from(App.getContext()).inflate(R.layout.common_toast_layout, null));
            mToast.setText(text);
    ...
    public static Toast getToast(int resId, int duration) { // 连续调用会连续弹出
        return getToast(App.getContext().getResources().getText(resId).toString(), duration);
    }

    public static Toast getToast(String text, int duration) {
        return Toast.makeText(App.getContext(), text, duration);
    }
}